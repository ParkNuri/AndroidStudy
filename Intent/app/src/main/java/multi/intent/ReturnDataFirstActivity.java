...
public class ReturnDataFirstActivity extends AppCompatActivity
    ...
    public static final int SECOND_BUTTON = 0000;
	...
	public void onClick(View v) {
		if(v.getId()==R.id.btnExit){
            ...
            intent.putExtra("data","msg form first!");

            // 되돌아올때 사용되는 메소드가 startActivityForResult
            // intent 객체와 함께 Request code를 넘긴다. (사용자 정의)
            // Request code로 호출한 객체 구분 가능 (출발점 구분)
            // 클래스 첫 줄에 상수 명 정의 :: public static final int SECOND_BUTTON = 0000;
            startActivityForResult(intent, SECOND_BUTTON);
        }
	}

	// intent를 통해서 activity를 호출하고 되돌아오는 경우 자동으로 onActivityResult를 호출한다.
    // 되돌아와서 마무리해야 하므로 onActivityResult를 오버라이딩해서 처리할 작업을 구현
    // requestCode - 요청을 했던 view를 구분하기 위한 코드
    // resultCode - 결과 코드
    // data - Intent 객체
