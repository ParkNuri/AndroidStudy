...
public class RuntimePermissionTest extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(ContextCompat.checkSelfPermission(this, Manifest.permission.CAMERA)== PackageManager.PERMISSION_GRANTED){
            ...
            // 2. 권한이 없는 경우 권한 설정 메세지 띄우기
            // requestPermissions( 어느 activity에 띄울지, 설정 권한, 사용자정의 코드 );
            ActivityCompat.requestPermissions(this, new String[]{Manifest.permission.CAMERA},1000);
        }
    }

    // 3. requestPermissions의 메세지창에서 선택한 후 호출되는 메소드
    // 결과를 return - 결과에 따라 다르게 처리할 수 있도록 구현
    @Override
    ...
    public void runCamera(View v){
        if(permission_state==true){
            ...
            /* 위험 권한 설정시 카메라 작동 x & 튕김 */
            startActivity(intent);
        }
        else {
            printToast("권한을 설정해야 이 기능을 사용할 수  있습니다");
            // 권한을 설정할 수 있는 액티비티로 자동 이동
        }
    }
}
